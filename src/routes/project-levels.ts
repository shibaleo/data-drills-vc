...
const app = new Hono()
  ...
  .patch("/reorder", zValidator("json", reorderInputSchema), async (c) => {
    ...
    await Promise.all(
      ids.map((id, i) =>
        db.update(level).set({ sortOrder: i, updatedAt: new Date() }).where(eq(level.id, id)),
      ),
    );
  ...
  .put("/:entityId", zValidator("json", masterUpdateInputSchema), async (c) => {
    ...
    const [row] = await db.update(level).set(updates).where(eq(level.id, c.req.param("entityId"))).returning();
    if (!row) return c.json({ error: "Not found" }, 404);
    return c.json({ data: row });
  })
  .delete("/:entityId", async (c) => {
    const [row] = await db.delete(level).where(eq(level.id, c.req.param("entityId"))).returning();
    if (!row) return c.json({ error: "Not found" }, 404);
    return c.json({ data: row });
  });
